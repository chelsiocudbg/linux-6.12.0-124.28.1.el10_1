...
void irdma_free_qp_rsrc(struct irdma_qp *iwqp)
{
	...
	SVC_DMA_FREE(rf->sc_dev.hw->device,
	...
	SVC_KFREE(iwqp->kqp.sq_wrid_mem,(void *)&iwqp->ibqp);
	SVC_KFREE(iwqp->kqp.rq_wrid_mem,(void *)&iwqp->ibqp);
	SVC_KFREE(iwqp->sg_list,(void *)&iwqp->ibqp);
}
...
#define IRDMA_CREATE_QP_MIN_RESP_LEN offsetofend(struct irdma_create_qp_resp, rsvd)
	...
	iwqp->sg_list = SVC_KMALLOC((uk_attrs->max_hw_wq_frags * sizeof(*iwqp->sg_list)), FLAG_IRDMA_FRAG, (void *)&iwqp->ibqp);
	if (!iwqp->sg_list)
		return -ENOMEM;
	...
	iwqp->q2_ctx_mem.size = ALIGN(IRDMA_Q2_BUF_SIZE + IRDMA_QP_CTX_SIZE,
				      256);
	iwqp->q2_ctx_mem.va = SVC_DMA_ZALLOC(dev->hw->device, iwqp->q2_ctx_mem.size,
		&iwqp->q2_ctx_mem.pa, GFP_KERNEL,(void *)&iwqp->ibqp);
...
#define IRDMA_CREATE_QP_MIN_RESP_LEN offsetofend(struct irdma_create_qp_resp, rsvd)
	...
	iwqp = SVC_KMALLOC(sizeof(*iwqp), FLAG_IRDMA_QP, (void *)&iwqp->ibqp);
	if (!iwqp)
		return ERR_PTR(-ENOMEM);
